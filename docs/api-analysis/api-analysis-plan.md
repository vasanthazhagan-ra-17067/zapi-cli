...
| Step | Action | Account |
|------|--------|---------|
| 1 | `zapi trace start --name api-analysis-<timestamp>` | vasanthseeker |
| 2 | Scope check — verify `ZohoCliq.Profile.CREATE`, `ZohoCliq.Profile.READ`, `ZohoCliq.Profile.DELETE` are present; add any missing via `zapi account scope add` and `zapi account refresh` | vasanthseeker |
...
| 9 | `zapi trace close --name api-analysis-<timestamp>` | vasanthseeker |
